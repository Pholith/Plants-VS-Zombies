...
public abstract class Zombie extends LivingEntity {
	...
	private final float eatDelay = 2f;
	...
	private final double timeSlowDelay = 7;
	...
	protected boolean conditionToEat(LivingEntity firstEnemy) {
		return !hypnotised && firstEnemy.getPosition().getX() > this.getPosition().getX() - 0.5 && (firstEnemy.isPlant() && !((Plant) firstEnemy).hasLadder()) || hypnotised && firstEnemy.getPosition().getX() < this.getPosition().getX() + 0.5;
	}
